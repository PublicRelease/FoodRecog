...
class MyModel(nn.Module):
    ...
    def forward(self, x, y):  # x:image, y:ingredient
        ...
        predicts, predicts_t = self.get_predicts_with_align(x_latent[:, 0:blk_len].cuda(3),
        ...
        return predicts.cuda(), predicts_t.cuda(), embed_vector.cuda(), multi_attention.cuda(), y_embeds.cuda(), x_recon.cuda(), y_recon.cuda(), wordPredicts.cuda(), x_latent[
                                                                                                                                                                      ...
                                                                                                                                                                      0:blk_len].cuda(), att_y_latent[
                                                                                                                                                                                         :,
                                                                                                                                                                                         0:blk_len].cuda(), encoder_outcome_recon.cuda(), wordPredict_rec.cuda(), decoder_y_recon.cuda()
...
def loss_function(predicts_V, predicts_T, labels, data, ingredients, embed_vector, multi_attention, y_embeds, x_recon,
                  ...
                  y_latent, encoder_outcome_recon, wordPredict_rec, decoder_y_recon,
                  ):
    # image channel loss
    ...
    return CE_V, CE_T, RE_V, lstm_loss, decoder_embed_match, ATT, encoder_outcome_recon_loss, decoder_outcome_recon_loss, reclstm_loss
...
def train(epoch):
    ...
    for batch_idx, (data, labels, ingredients, indexVector) in enumerate(train_loader):
        ...
        predicts_V, predicts_T, embed_vector, multi_attention, y_embeds, x_recon, y_recon, wordPredicts, x_latent, y_latent, encoder_outcome_recon, wordPredict_rec, decoder_y_recon = model(
            data, indexVector)
        ...
        CE_V, CE_T, RE_V, lstm_loss, decoder_embed_match, AE, ATT, encoder_outcome_recon_loss, decoder_outcome_recon_loss, loss_z, loss_img, loss_text, reclstm_loss, recloss_tag2img = loss_function(
            ...
            x_latent, y_latent,
            encoder_outcome_recon, AE, wordPredict_rec, decoder_y_recon)
        ...
        if epoch == 1 and batch_idx == 0:
            print(
                'Train Epoch: {} [{}/{} ({:.0f}%)] | Loss: {:.4f} | CE_V: {:.4f} | CE_T: {:.4f} | RE_V: {:.4f} | recloss_tag2img: {:.4f} | lstm_loss: {:.4f} | reclstm_loss: {:.4f} | decoder_embed_match: {:.4f} | ATT: {:.4f} | encoder_outcome_recon_loss: {:.4f} |  decoder_outcome_recon_loss: {:.4f} | Top1_Accuracy_V:{} | Top5_Accuracy_V:{} | Time:{} | Total_Time:{}'.format(
                    ...
                    ATT.data, encoder_outcome_recon_loss.data, decoder_outcome_recon_loss.data,
