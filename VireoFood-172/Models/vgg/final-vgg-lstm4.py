...
class MyModel(nn.Module):
    ...
    def forward(self, x, y):  # x:image, y:ingredient
        ...
        # get img recon
        img_recon = self.decoder(x_latent_recon.cuda(set_gpu_decoder))
        # get img recon features
        img_recon_feature = self.get_latent(self.encoder(x.cuda(set_gpu_encoder)).cuda(set_gpu_others))

        return x_latent.cuda(set_gpu_others), x_latent_recon.cuda(set_gpu_others), img_recon.cuda(
            set_gpu_others), img_recon_feature.cuda(set_gpu_others)
...
def get_optim(lr):
    ...
    frozen_params = [p for n, p in model.named_parameters() if
                     n.startswith('encoder.') or n.startswith('vgg_') or n.startswith('encoder_t.')]
    # finetune_params = [p for n, p in model.named_parameters() if n.startswith('decoder_t.')]
    non_frozen_params = [p for n, p in model.named_parameters() if n.startswith('cross_') or n.startswith('decoder.')]
...
def loss_function(x_latent, x_latent_recon, img_recon_feature, img_recon, data):
    # cross channel x_latent_recon_loss
    x_latent_recon_loss = torch.sum((x_latent - x_latent_recon) ** 2)

    # image reconstruction loss
    img_recon_l2_loss = torch.sum((img_recon - data.cuda(set_gpu_others)) ** 2)
    # image recon feature loss
    img_recon_feature_loss = torch.sum((img_recon_feature - x_latent) ** 2)

    return x_latent_recon_loss, img_recon_l2_loss, img_recon_feature_loss
...
def train(epoch):
    ...
    for batch_idx, (data, labels, ingredients, indexVector) in enumerate(train_loader):
        ...
        x_latent, x_latent_recon, img_recon, img_recon_feature = model(data, indexVector)

        # loss
        x_latent_recon_loss, img_recon_l2_loss, img_recon_feature_loss = loss_function(x_latent, x_latent_recon,
                                                                                       img_recon_feature, img_recon,
                                                                                       data)
        ...
        if epoch == 1 and batch_idx == 0:
            print(
                'Train Epoch: {} [{}/{} ({:.0f}%)] | Loss: {:.4f} | x_latent_recon_loss: {:.4f} | img_recon_l2_loss: {:.4f} | img_recon_feature_loss: {:.4f} | Top1_Accuracy_V:{} | Top5_Accuracy_V:{} | Time:{} | Total_Time:{}'.format(
                    epoch, (batch_idx + 1) * labels.shape[0], len(train_loader.dataset),
                           100. * (batch_idx + 1) / len(train_loader), loss.data,
                    x_latent_recon_loss, img_recon_l2_loss, img_recon_feature_loss,
