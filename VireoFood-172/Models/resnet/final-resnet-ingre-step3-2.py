...
premodel_path2 = root_path + 'results_resnet-ingre-step3/'

result_path = root_path + 'results_resnet-ingre-step3-2/'
...
class MyModel(nn.Module):
    def __init__(self, num_classes=172):
        ...
        # self.img_feature = resnet18(pretrained=True).cuda(set_gpu_others)
    ...
    def forward(self, x, y):  # x:image y:ingredient
        ...
        return predicts_v.cuda(set_gpu_others), predicts_t.cuda(set_gpu_others), AE_l2.cuda(set_gpu_others), AE_kl.cuda(
            set_gpu_others)
    ...
    def get_y_latent(self, y):
        ...
        #     x_fea, _ = self.img_feature(x.cuda(set_gpu_others))
        #     x_recon_fea, _ = self.img_feature(x_recon.cuda(set_gpu_others))
        #
...
def loss_function(predicts_v, predicts_t, labels):
    CE_V = criterion(predicts_v, labels - 1) * 2e1
    CE_T = criterion(predicts_t, labels - 1) * 1e1
...
def train(epoch):
    ...
    for batch_idx, (data, labels, ingredients) in enumerate(
        ...
        predicts_v, predicts_t, AE_l2, AE_kl = model(data, ingredients)

        # calculate scalar loss
        CE_V, CE_T = loss_function(predicts_v, predicts_t, labels.cuda(set_gpu_others))
        # calculate the gradient of the loss w.r.t. the graph leaves

        # decrease by 10 if CE_V fails to converge
        AE_l2 *= 1e-1
        AE_kl *= 1e2
        loss = CE_V + CE_T + AE_l2 + AE_kl
        loss.backward()
        ...
        if epoch == 1 and batch_idx == 0:
            # compute acc
            top1_accuracy_cur_V, top1_accuracy_cur_T, top5_accuracy_cur_V, top5_accuracy_cur_T = top_n_acc(predicts_v,
                                                                                                           predicts_t,
                                                                                                           labels)

            print(
                'Train Epoch: {} [{}/{} ({:.0f}%)] | Loss: {:.4f} | CE_V: {:.4f} | CE_T: {:.4f} | AE_l2: {:.4f} | AE_kl: {:.4f} | Top1_Accuracy_V:{} | Top5_Accuracy_V:{} | Top1_Accuracy_T:{} | Top5_Accuracy_T:{} | Time:{} | Total_Time:{}'.format(
                    epoch, (batch_idx + 1) * len(data), len(train_loader.dataset),
                           100. * (batch_idx + 1) / len(train_loader), loss.data,
                    CE_V, CE_T, AE_l2, AE_kl,
