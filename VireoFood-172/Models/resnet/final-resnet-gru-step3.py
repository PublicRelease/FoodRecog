...
class MyModel(nn.Module):
    ...
    def forward(self, x, y):  # x:image, y:ingredient
        ...
        return embed_vector.cuda(), multi_attention.cuda(), y_embeds.cuda(), x_recon.cuda(), y_recon.cuda(), wordPredicts.cuda(), x_latent[
                                                                                                                                                                      :,
                                                                                                                                                                      0:blk_len].cuda(), att_y_latent[
                                                                                                                                                                                         :,
                                                                                                                                                                                         0:blk_len].cuda(), encoder_outcome_recon.cuda(), loss_z.cuda(), loss_img.cuda(), loss_text.cuda(), wordPredict_rec.cuda(), decoder_y_recon.cuda(), rec_imgLatent.cuda()
...
def loss_function(data, ingredients, embed_vector, multi_attention, y_embeds, x_recon,
                  y_recon, wordPredicts, x_latent,
                  y_latent, encoder_outcome_recon, loss_z, loss_img, loss_text, wordPredict_rec, decoder_y_recon,
                  rec_imgLatent):
    ...
    return RE_V, lstm_loss, decoder_embed_match, AE, ATT, encoder_outcome_recon_loss, decoder_outcome_recon_loss, loss_z, loss_img, loss_text, reclstm_loss, recloss_tag2img
...
def train(epoch):
    ...
    for batch_idx, (data, labels, ingredients, indexVector) in enumerate(train_loader):
        ...
        embed_vector, multi_attention, y_embeds, x_recon, y_recon, wordPredicts, x_latent, y_latent, encoder_outcome_recon, loss_z, loss_img, loss_text, wordPredict_rec, decoder_y_recon, rec_imgLatent = model(
            data, indexVector)

        # loss
        RE_V, lstm_loss, decoder_embed_match, ATT, encoder_outcome_recon_loss, decoder_outcome_recon_loss, loss_z, loss_img, loss_text, reclstm_loss, recloss_tag2img = loss_function(
            data, ingredients,
            embed_vector,
            multi_attention,
            y_embeds, x_recon,
        ...
        optimizer.zero_grad()
        loss = RE_V + lstm_loss + decoder_embed_match + ATT + encoder_outcome_recon_loss + decoder_outcome_recon_loss + loss_z + loss_img + loss_text + reclstm_loss + recloss_tag2img
        loss.backward()
        ...
        if epoch == 1 and batch_idx == 0:
            print(
                'Train Epoch: {} [{}/{} ({:.0f}%)] | Loss: {:.4f} | RE_V: {:.4f} | recloss_tag2img: {:.4f} | lstm_loss: {:.4f} | reclstm_loss: {:.4f} | decoder_embed_match: {:.4f} | ATT: {:.4f} | encoder_outcome_recon_loss: {:.4f} |  decoder_outcome_recon_loss: {:.4f} | loss_z: {:.4f} | loss_img: {:.4f} | loss_text: {:.4f} | Top1_Accuracy_V:{} | Top5_Accuracy_V:{} | Time:{} | Total_Time:{}'.format(
                    epoch, (batch_idx + 1) * len(data), len(train_loader.dataset),
                           100. * (batch_idx + 1) / len(train_loader),
                    loss.data, RE_V.data, recloss_tag2img.data, lstm_loss.data, reclstm_loss.data,
                    decoder_embed_match.data,
                    ATT.data, encoder_outcome_recon_loss.data, decoder_outcome_recon_loss.data, loss_z.data,
