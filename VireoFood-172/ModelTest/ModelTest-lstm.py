...
result_path = root_path + 'results3/'

past_result_path = root_path + 'past_results/final-vgg-lstm2-2/'
...
class FoodData(torch.utils.data.Dataset):
    def __init__(self, train_data=False, test_data=False, transform=None,
        ...
        if train_data:
            ...
            indexVector = matio.loadmat(file_path + 'indexVector_test2.mat')['indexVector_test2']
...
class MyModel(nn.Module):
    ...
    def forward(self, y):  # x:image, y:ingredient
        ...
        return gru_predicts.cuda(set_gpu_others)
...
def test():
    ...
    for test_batch_idx, (data, wordIndicator_test) in enumerate(test_loader
        ...
        if CUDA:
            ...
            data = data.cuda(set_gpu_others)

        # predicts_V, x = model(data)
        wordPredicts = model(data)
        wordPredicts = wordPredicts.transpose(0, 1)
...
for i in range(start, end + 1):
    # path = file_path + 'finalModel_lstm.pt'
    path = past_result_path + 'model' + str(i) + '.pt'
    model = get_updateModel(path)

    avg_precision, avg_recall, avg_precision_word = test()

    print('saving result to {}'.format(result_path + 'img2tag_performance.txt'))
    with io.open(past_result_path + 'img2tag_performance.txt', 'a', encoding='utf-8') as file:
        file.write(
